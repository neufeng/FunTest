...
CGFloat const kTWMessageViewIconSize = 22.0f;
...
//    [self messageWindowView];
//    [self.messageWindow addSubview:messageView];
...
- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type inView:(UIView *)aView
{
    [self showMessageWithTitle:title description:description type:type duration:kTWMessageBarManagerDisplayDelay callback:nil inView:aView];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type duration:(CGFloat)duration callback:(void (^)())callback inView:(UIView *)aView
{
    TWMessageView *messageView = [[TWMessageView alloc] initWithTitle:title description:description type:type];
    messageView.delegate = self;
    ...
    messageView.statusBarStyle = UIStatusBarStyleDefault;
    messageView.statusBarHidden = NO;
    
    [aView addSubview:messageView];
    [aView bringSubviewToFront:messageView];
    
    [self.messageBarQueue addObject:messageView];
    
    if (!self.messageVisible)
    {
        [self showNextMessageInView];
    }
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type customView:(UIView *)aView
{
    [self showMessageWithTitle:title description:description type:type duration:kTWMessageBarManagerDisplayDelay customView:aView];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type duration:(CGFloat)duration customView:(UIView *)aView
{
    [self showMessageWithTitle:title description:description type:type duration:duration callback:nil customView:aView];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type duration:(CGFloat)duration callback:(void (^)())callback customView:(UIView *)aView
{
    [self showMessageWithTitle:title description:description type:type duration:duration callback:callback customView:aView animated:YES];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type customView:(UIView *)aView animated:(BOOL)animated
{
    [self showMessageWithTitle:title description:description type:type duration:kTWMessageBarManagerDisplayDelay customView:aView animated:animated];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type duration:(CGFloat)duration customView:(UIView *)aView animated:(BOOL)animated
{
    [self showMessageWithTitle:title description:description type:type duration:duration callback:nil customView:aView animated:animated];
}

- (void)showMessageWithTitle:(NSString *)title description:(NSString *)description type:(TWMessageBarMessageType)type duration:(CGFloat)duration callback:(void (^)())callback customView:(UIView *)aView animated:(BOOL)animated
{
    TWMessageView *messageView = [[TWMessageView alloc] initWithTitle:title description:description type:type];
    [messageView addSubview:aView];
    messageView.delegate = self;
    ...
    if (!self.messageVisible)
    {
        [self showNextMessageAnimate:animated];
    }
}
...
//            yOffset = 20.0;
//        }
        messageView.frame = CGRectMake(0, -[messageView height]-yOffset, [messageView width], [messageView height]);
        messageView.hidden = NO;
        [messageView setNeedsDisplay];
        NSLog(@"message %@", NSStringFromCGRect(messageView.frame));
...
- (void)showNextMessageAnimate:(BOOL)animated
{
    if ([self.messageBarQueue count] > 0)
    {
        self.messageVisible = YES;
        ...
        if (animated) {
            ...
            messageView.frame = CGRectMake(0, -yOffset, [messageView width], [messageView height]);
        }
        messageView.hidden = NO;
        [messageView setNeedsDisplay];
...
- (void)generateAccessibleElementWithTitle:(NSString *)title description:(NSString *)description
{
    ...
    if (title.length < 1) {
        textElement.accessibilityLabel = [NSString stringWithFormat:@"%@", description];
    }
...
- (TWMessageBarViewController *)messageBarViewController
{
    ...
    {
        self.messageWindow = [[TWMessageWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
//        self.messageWindow.windowLevel = UIWindowLevelNormal;
        ...
        self.messageWindow.rootViewController.wantsFullScreenLayout = YES;
#endif
        self.messageWindow.hidden = NO;
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.messageType == TWMessageBarMessageTypeLocateInfo || self.messageType == TWMessageBarMessageTypeLocateSuccess) {
        [super drawRect:rect];
        return;
    }
...
//        CGContextSaveGState(context);
//        {
//            UIImage *glassImage = [UIImage imageNamed:@"glass_bg.png"];
//            [glassImage drawInRect:rect];
//        }
//        CGContextRestoreGState(context);
        ...
        CGFloat xOffset = (rect.size.width - titleLabelSize.width)/2 - kTWMessageViewIconSize - kTWMessageViewBarPadding;
        CGFloat yOffset = (rect.size.height - kTWMessageViewIconSize)/2;
...
//        CGContextRestoreGState(context);
        ...
        yOffset = kTWMessageViewBarPadding + [self statusBarOffset];
        xOffset = 0;
        ...
        if ([[UIDevice currentDevice] isRunningiOS7OrLater])
        {
            if (self.descriptionString.length < 1) {
                xOffset = 0;
                yOffset = (rect.size.height - titleLabelSize.height)/2;
                
                NSMutableParagraphStyle *paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
                paragraphStyle.alignment = NSTextAlignmentCenter;
                
                [[self titleColor] set];
                [self.titleString drawWithRect:CGRectMake(xOffset, yOffset, rect.size.width, titleLabelSize.height)
                                       options:(NSStringDrawingOptions)(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine)
            ...
            else if (self.titleString.length < 1) {
                xOffset = 0;
                yOffset = (rect.size.height - descriptionLabelSize.height)/2;
                
                NSMutableParagraphStyle *paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
                paragraphStyle.alignment = NSTextAlignmentCenter;
                
                [[self descriptionColor] set];
                [self.descriptionString drawWithRect:CGRectMake(xOffset, yOffset, rect.size.width, descriptionLabelSize.height)
                                             options:(NSStringDrawingOptions)(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine)
        ...
        else
        {
            if (self.descriptionString.length < 1) {
                ...
                [[self titleColor] set];
                [self.titleString drawInRect:CGRectMake(0, yOffset, rect.size.width, rect.size.height) withFont:[self titleFont] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
            }
            else if (self.titleString.length < 1) {
                xOffset = 0;
                yOffset = (rect.size.height - descriptionLabelSize.height)/2;
                ...
                [[self descriptionColor] set];
                [self.descriptionString drawInRect:CGRectMake(0, yOffset, rect.size.width, rect.size.height) withFont:[self titleFont] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
            }
            else {
                [[self titleColor] set];
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                [self.titleString drawInRect:CGRectMake(xOffset, yOffset, titleLabelSize.width, titleLabelSize.height) withFont:[self titleFont] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                [self.descriptionString drawInRect:CGRectMake(xOffset, yOffset, descriptionLabelSize.width, descriptionLabelSize.height) withFont:[self descriptionFont] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
#pragma clang diagnostic pop
            }
        }
        
//        //  bottom line
//        UIColor *bottomLineColor = RGB(0x0988ea);
//        CGRect lineRect = CGRectMake(0, rect.size.height-2, rect.size.width, 2);
//        CGContextSaveGState(context);
//        {
//            [bottomLineColor set];
...
//    return MIN((kTWMessageViewBarPadding * 2) + titleLabelSize.height + descriptionLabelSize.height + [self statusBarOffset], 64);
    
//    return [[UIDevice currentDevice] isRunningiOS7OrLater] ? 64 : 44;
//    return 64;
...
- (CGSize)titleSize
{
    ...
    if ([[UIDevice currentDevice] isRunningiOS7OrLater])
    {
        ...
        titleLabelSize = [self.titleString boundingRectWithSize:boundedSize
                                                        options:(NSStringDrawingOptions)(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin)
